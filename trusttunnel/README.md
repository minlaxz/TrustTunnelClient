...
Options:
  ...
  -e, --endpoint_config <path>
      ...
      --subscription-url <url>
                              Subscription URL to import the server parameters from (conflicts with -e, -a, -n, -c)
      --name <name>           Display name of the server entry
      --dns <upstream>        DNS upstream, e.g. tls://8.8.8.8 (repeatable)
  -h, --help                  Print help
```
...
| `name` | string | `null` | Human-readable server name. Written on server creation only |
| `subscription.url` | string | `null` | HTTPS URL of the subscription document, optionally with embedded HTTP Basic Auth credentials (`https://user:pass@host/path`). Fresh endpoint parameters are pulled from it on `--refresh` |
| `subscription.last_fetched_at` | string | `null` | Time of the last successful fetch (RFC 3339 UTC). Absent until the first successful refresh |
...
### Refreshing Server Configuration

If the config contains an `[endpoint.subscription]` URL, fetch the latest
endpoint parameters and update the file in-place (does not start the VPN):

```shell
./trusttunnel_client --refresh --config <path/to/configuration/file.toml>
```

A running client reloads its configuration on `SIGHUP`, so after a successful
refresh you can signal the process to reconnect with the updated parameters:
...
To keep the configuration current automatically, schedule a refresh and signal
the running client afterwards. Example cron entry:

```shell
/opt/trusttunnel_client/trusttunnel_client --refresh --config /opt/trusttunnel_client/trusttunnel_client.toml && kill -HUP $(pidof trusttunnel_client)
```
